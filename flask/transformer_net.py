...
class TransformerNet(nn.Module):
    ...
    def forward(self, x):
        out = self.softplus(self.ins1(self.conv1(x)))
        out = self.softplus(self.ins2(self.conv2(out)))
        out = self.softplus(self.ins3(self.conv3(out)))
        out = self.res1(out)
        out = self.res2(out)
        out = self.res3(out)
        out = self.res4(out)
        ...
        out = self.res6(out)
        out = self.res7(out)
        out = self.softplus(self.ins4(self.deconv1(out)))
        out = self.softplus(self.ins5(self.deconv2(out)))
